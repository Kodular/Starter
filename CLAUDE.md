...
- **`components/`** — Small UI primitives (Button, Field, Input, Select, IconButton) with Tailwind v4 styling

### Backend (`src-tauri/src/`)

- **`lib.rs`** — Tauri command handlers exposed to the frontend. Spawns the axum server on startup.
- **`adb_commands.rs`** — ADB logic. Supports three modes via `AdbMode` enum:
  - `Auto`: tries system ADB first, falls back to built-in USB (`adb_client` crate)
  - `System`: only uses system `adb` binary (via `ADBServer`)
...
- **`settings.rs`** — Reads/writes `AppSettings` (adb_mode + custom_adb_path) using `tauri-plugin-store`
